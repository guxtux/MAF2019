...
\item Las correspondientes eigenfunciones son los \textocolor{cobalt}{armónicos esféricos}.
\end{enumerate}
\end{frame}
\begin{frame}
...
Sin embargo, \pause la forma más sencilla y elegante de obtener las soluciones es mediante el uso de los \emph{\textocolor{darkorchid}{operadores de escalera}} del momento angular.
\end{frame}
\begin{frame}
...
\begin{enumerate}[<+->]
\item \textocolor{bole}{Polinomios asociados de Laguerre $L_{n}^{k} (r)$}.
\item Como caso especial, los \textocolor{darkred}{polinomios ordinarios de Laguerre $L_{n} (r)$}.
\end{enumerate}
\end{frame}
\begin{frame}
...
\subsection{Asociadas de Legendre para enteros \texorpdfstring{$\ell$}{l}}
