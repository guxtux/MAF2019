...
\begin{frame}
\frametitle{Estado estacionario}
En principio, si ponemos un átomo de hidrógeno en un estado estacionario $\Psi_{n \ell m}$, debe quedarse allí para siempre.
\end{frame}
\begin{frame}
\frametitle{Estado perturbado}
Sin embargo, si \textocolor{amethyst}{\enquote{perturbamos}} ligeramente (por colisión con otro átomo, por ejemplo, o haciéndole incidir  luz en él), \pause entonces el átomo puede experimentar una transición a otro estado estacionario mediante la absorción de la energía y pasarse a un estado de mayor energía o cediendo energía (normalmente en forma de radiación electromagnética) y moverse  hacia abajo.
\end{frame}
\begin{frame}
\frametitle{Estado perturbado}
En la práctica este tipo de perturbaciones están siempre presentes; transiciones (o, como a veces se denominan \enquote{\textocolor{auburn}{saltos cuánticos}}) se producen constantemente,
\end{frame}
\begin{frame}
\frametitle{Estado perturbado}
...
\end{frame}
\begin{frame}
\frametitle{Relación entre energía y frecuencia}
En acuerdo con la fórmula de Planck, la energía de un fotón es proporcional a su frecuencia:
\pause
\begin{align}
E_{\gamma} = h \, \nu
...
\end{frame}
\begin{frame}
\frametitle{Relación con la longitud de onda}
...
\end{frame}
\begin{frame}
\frametitle{Espectro de hidrógeno}
Donde a $R$ se le conoce como la \textbf{constante de Rydberg}, \pause y la ec. (\ref{eq:ecuacion_04_93}) es la \textbf{fórmula Rydberg} para el espectro de hidrógeno.
\end{frame}
\begin{frame}
\frametitle{Espectro de hidrógeno}
...
\end{frame}
\begin{frame}
\frametitle{Transiciones al estado base}
Las transiciones al estado base ($n_{f}= 1$) se encuentran en el ultravioleta; \pause son conocidas por los espectroscopistas como la \textbf{serie de Lyman}.
\end{frame}
\begin{frame}
\frametitle{Transiciones al estado base}
Las transiciones al primer estado excitado ($n_{f}= 2$) se encuentran en la zona de región visible; conforman la \textbf{serie de Balmer}.
\end{frame}
\begin{frame}
\frametitle{Transiciones al estado base}
Las transiciones a $n_{f} = 3$ (la \textbf{serie de Paschen}) están en el infrarrojo, y así sucesivamente (véase la figura \ref{fig:figura_espectro_H}).
\end{frame}
\begin{frame}
\frametitle{Transiciones al estado base}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\linewidth]{Imagenes/espectrohidrogeno.eps}
