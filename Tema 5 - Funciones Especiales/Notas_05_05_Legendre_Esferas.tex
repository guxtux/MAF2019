...
\bigskip
\pause
Pero en el camino encontramos \textocolor{carmine}{dos constantes de separación}: $\alpha$ y $\beta$.
\end{frame}
\begin{frame}
...
\end{align}
que es la \textocolor{carnelian}{ecuación radial}.
\end{frame}
\begin{frame}
...
\end{align}
esta es la \textocolor{coolblack}{ecuación polar}.
\end{frame}
\begin{frame}
...
\end{align}
es la llamada \textocolor{darkmagenta}{ecuación azimutal.}
\pause
...
En casos donde sea conocido de antemano que el potencial sea independiente del ángulo azimutal $\varphi$, entonces el problema presenta una \textocolor{darkscarlet}{simetría azimutal}.
\end{frame}
\begin{frame}
...
\end{align}
\pause
A esta ecuación se le conoce como la \textocolor{darkslateblue}{ecuación diferencial de Legendre}.
\end{frame}
\begin{frame}
...
\begin{frame}
\frametitle{La solución al problema}
Que \textocolor{blue}{determina la temperatura en puntos interiores de la esfera}:
\pause
\begin{eqnarray*}
\begin{aligned}
&T (r, \theta) = \\[0.5em]
...
\pause
\setbeamercolor{item projected}{bg=blue,fg=bananayellow}
