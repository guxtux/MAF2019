...
\pause
\begin{align*}
&F^{-1} \, \big[F (\xi) \, G (\xi); \xi \to x\big] = \\[1em]
&= \dfrac{1}{\sqrt{2 \, \pi}} \scaleint{6ex}_{\bs -\infty}^{+\infty} f (x - u) \, g (u) \dd{u} 
\end{align*}
\end{frame}
\begin{frame}
\frametitle{Corolario 1 al Teorema}
...
\end{align*}
\pause
Que es la función de temperatura
\end{frame}
\begin{frame}
...
&+ \dfrac{1}{\sqrt{2 \pi}} \scaleint{6ex}_{-\infty}^{+\infty} \overline{f} (\xi) \cdot \exp(- \xi (x + c t)) \dd{\xi} \bigg] \\[0.5em] \pause
...
\begin{frame}
\frametitle{Tercer enunciado}
% \textbf{Ejercicio a cuenta (59). }
La temperatura $u (x, t)$ en una barra semiinfinita $0 \leq x < \infty$ satisface la siguiente EDP:
\begin{align*}
...
\pdv{u}{x} &= - \lambda \hspace{0.5cm} \mbox{una constante, cuando } x = 0, \hspace{0.2cm} t > 0  
\end{align*}
