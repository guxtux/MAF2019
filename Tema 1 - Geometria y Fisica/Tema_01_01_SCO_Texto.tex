...
\end{figure}
Para obtener las superficies coordenadas, hay que apoyarnos con lo que ya sabemos sobre geometría del espacio, por lo que deberemos de recordar las expresiones generales que nos definen planos, esferas, conos, cilindros, etc.; el ejercicio para el sistema esférico se desarrolló en clase y lo puedes nuevamente trabajar en tu cuaderno.
\par
...
\begin{align*}
\abs{\pdv{\vb{r}}{r}} &= \sqrt{\sin^{2} \theta \, \cos^{2} \varphi + \sin^{2} \theta \, \sin^{2} \varphi + \cos^{2} \theta} \\[0.5em]
&= 1
\end{align*}
...
Entonces:
\begin{align*}
\abs{\pdv{\vb{r}}{\theta}} &= \sqrt{r^{2} (\cos^{2} \theta \, \cos^{2} \varphi + \cos^{2} \theta \, \sin^{2} \varphi + \sin^{2} \theta)} \\[0.5em]
&= r
\end{align*}
\par
Así: $h_{2} = h_{\theta} = r$
...
Entonces:
\begin{align*}
\abs{\pdv{\vb{r}}{\varphi}} &= \sqrt{r^{2} (\sin^{2} \theta \, \sin^{2} \varphi + \sin^{2} \theta \, \cos^{2} \varphi)} \\[0.5em]
&= r \, \sin \theta
\end{align*}
\par
...
En el caso de una rotación, o del paso de coordenadas cartesianas a curvilíneas, puesto que la matriz de transformación ha de contener la identidad, entonces \break \hfill $\abs{\vb{A}} = +1$. Para el caso de la reflexión e inversión, se tiene $\abs{\vb{A}} = -1$.
