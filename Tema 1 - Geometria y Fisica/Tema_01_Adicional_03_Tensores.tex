...
\begin{align}
\va{E}_{i} \vdot \va{E}_{j} = g_{ij} = g_{ji}  \hspace{1cm} \text{y} \hspace{1cm} \va{E}^{i} \vdot \va{E}^{j} = g^{ij} = g^{ji}
...
\mbox{o de la forma} \hspace{0.5cm} &{} \\[0.5em]
...
Por ejemplo, el conjunto de elementos $G = \left\{ 1, -1, i, -i \right\}$, donde $i^{2} = -1$ junto con la ley de combinación de la multiplicación ordinaria, forma un grupo. Esto se puede ver en la siguiente tabla de multiplicación:
\begin{table}[H]
\centering
...
\section{Transformación vectorial.}
...
\\
\noindent
\encabezaejemplo Propiedad transitiva de la transformación contravariante.

Demuestra que una sucesión de transformaciones contraviariantes es también una transformación contravariante.

\noindent
\textbf{Solución: } Considera la tranformación de un vector de un sistema sin barra a un sistema coordenado con barras. Un vector o tensor de rango uno \break \hfill $A^{i} = A^{i} (x) = 1, \ldots, N$ será una transformación como en la ecuación (\ref{eq:ecuacion_01_02_40}) y
\begin{align}
    \bar{A}^{i} (\bar{x}) = \pdv{\bar{x}^{i}}{x^{j}} \, A^{j} (x)
    \label{eq:ecuacion_01_02_42}
\end{align}
Otra transformación de coordenadas de $\bar{x} \to \bar{\bar{x}}$, genera los componentes
\begin{align}
    \bar{\bar{A}}^{i} (\bar{\bar{x}}) = \pdv{\bar{\bar{x}}^{i}}{\bar{x}^{j}} \, \bar{A}^{j} (x)
    \label{eq:ecuacion_01_02_43}
\end{align}
Aquí se ha usado la notación $A^{j} (x)$ para enfatizar la dependencia de los componentes $A^{j}$ sobre las coordenadas $x$. Cambiando los índices y sustituyendo la ecuación (\ref{eq:ecuacion_01_02_42}) en la (\ref{eq:ecuacion_01_02_43}), se encuentra que
\begin{align}
    \bar{\bar{A}}^{i} (\bar{\bar{x}}) = \pdv{\bar{\bar{x}}^{i}}{\bar{x}^{j}} \, \pdv{\bar{x}^{j}}{x^{m}} \, A^{m} (x)
    \label{eq:ecuacion_01_02_44}
\end{align}
Del hecho que
\begin{align*}
    \pdv{\bar{\bar{x}}^{i}}{\bar{x}^{j}} \, \pdv{\bar{x}^{j}}{x^{m}} = \pdv{\bar{\bar{x}}^{i}}{x^{m}}
\end{align*}
la ecuación (\ref{eq:ecuacion_01_02_44}) se simplifica como
\begin{align}
    \bar{\bar{A}}^{i} (x) = \pdv{\bar{\bar{x}}^{i}}{x^{m}} \, A^{m} (x)
    \label{eq:ecuacion_01_02_45}
\end{align}
y de aquí que la transformación sea también contravariante. Esto debido a que la expresión es transitiva respecto al grupo de transformación de coordenadas.
\par
Notemos que de la regla de la cadena, podemos escribir
\begin{align*}
    \pdv{x^{m}}{\bar{x}^{j}} \, \pdv{\bar{x}^{j}}{x^{n}} = \pdv{x^{m}}{\bar{x}^{1}} \, \pdv{\bar{x}^{1}}{x^{n}} +  \pdv{x^{m}}{\bar{x}^{2}} \, \pdv{\bar{x}^{2}}{x^{n}} + \pdv{x^{m}}{\bar{x}^{3}} \, \pdv{\bar{x}^{3}}{x^{n}} = \pdv{x^{m}}{x^{n}} = \delta_{n}^{m}
\end{align*}
No debemos de equivocarnos escribiendo
\begin{align*}
    \pdv{x^{m}}{\bar{x}^{2}} \, \pdv{\bar{x}^{2}}{x^{n}} = \pdv{x^{m}}{x^{n}} \hspace{0.75cm} \text{o} \hspace{0.75cm} \pdv{x^{m}}{\bar{x}^{3}} \, \pdv{\bar{x}^{3}}{x^{n}} = \pdv{x^{m}}{x^{n}}
\end{align*}
ya que las expresiones son incorrectas. Revisemos que no hay sumas en esas expresiones, mientras que hay un índice de suma en la representación de la regla de la cadena.
...
\\
\noindent
\encabezaejemplo Propiedad transitiva de la transformación covariante.

Considera una secuencia de transformaciones definidas por la ecuación (\ref{eq:ecuacion_01_02_47})
\begin{align*}
    x &\to \bar{x} \hspace{1cm} \bar{A}_{i} (\bar{x}) = A_{j} (x) \, \pdv{x^{j}}{\bar{x}^{i}} \\[1em]
    x &\to \bar{\bar{x}} \hspace{1cm} \bar{\bar{A}}_{i} (\bar{x}) = \bar{A}_{m} (x) \, \pdv{\bar{x}^{m}}{\bar{\bar{x}}^{k}}
\end{align*}
Por lo tanto, podemos expresar la transformación de los componentes asociados con la transformación de coordenadas $x \to \bar{\bar{x}}$ y
\begin{align*}
    \bar{\bar{A}}_{k} (\bar{\bar{x}}) = \left( A_{j} (x) \, \pdv{x^{j}}{\bar{x}^{m}} \right) \pdv{\bar{x}^{m}}{\bar{\bar{x}}^{m}} = A_{j} (x) \, \pdv{x^{j}}{\bar{\bar{x}}^{k}}
\end{align*}
lo cual demuestra la propiedad transitiva de una transformación covariante.
...
\end{align*}
Si una diádica es igual a su conjugada $\vb{A} = \vb{A}_{c}$, entonces $A_{ij} = A_{ji}$ y la diádica se llama \textbf{simétrica}. Si una diádica es igual al negativo de su conjugada $\vb{A} = - \vb{A}_{c}$, entonces $A_{ij} = - A_{ji}$ y la diádica se llama \textbf{antisimétrica}. Una diádica especial llamada \textbf{diádica idéntica o factor idéntico} que está definida por:
\begin{align*}
...
\noindent
\encabezaejemplo \textbf{Producto interno}. Sean $A^{i}$ y $B_{j}$ los componentes de dos tensores (vectores) de primer orden. El producto externo de estos tensores es:
\begin{align*}
