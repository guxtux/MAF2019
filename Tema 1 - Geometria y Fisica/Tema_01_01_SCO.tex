...
\begin{frame}
\frametitle{Sistema coordenado cilíndrico}
En la siguiente animación vemos las superficies coordenadas constantes y su intersección.
\end{frame}
\begin{frame}
...
\begin{figure}
   \centering
   \includegraphics[width=0.9\linewidth]{Imagenes/superficies_esfericas_04.png}
   \caption{}
...
\begin{frame}
\frametitle{Campo vectorial}
Aunque los vectores unitarios y los valores de cada componente sean diferentes en cada sistema de coordenadas, es sin embargo cierto que el vector $\va{A}$ no cambia cuando cambiamos de sistema coordenado.
\end{frame}
\begin{frame}
\frametitle{Campo vectorial}
...
\end{frame}
\begin{frame}
\frametitle{Una conexión: factores de escala y métrica}
\begin{block}{Para extender el estudio}
En un sistema de coordenadas ortogonal
$\left( u_{1}, u_{2}, u_{3} \right)$, los factores de escala
\pause
\begin{align*}
   h_{i} = \abs{\pdv{\vb{r}}{u_{i}}}
\end{align*}
\pause
expresan el elemento de desplazamiento como:
\pause
\begin{align*}
\dd{\vb{r}} = h_{1} \,\dd{u_{1}} \, \vu{e}_{1} + h_{2} \,\dd{u_{2}} \, \vu{e}_{2} + h_{3} \,\dd{u_{3}} \, \vu{e}_{3}
\end{align*}
\end{block}
\end{frame}
\begin{frame}
\frametitle{Factores de escala y métrica}
Al calcular $\dd{s^{2}} = \dd{\vb{r}} \cdot \dd{\vb{r}}$ se obtiene
\pause
\begin{align*}
\boxed{
   \dd{s^{2}} = h_{1}^{2} (\dd{u}_{1})^{2} + h_{2}^{2} (\dd{u}_{2})^{2} + h_{3}^{2} (\dd{u}_{3})^{2}
}
\end{align*}
\pause
Por tanto, en coordenadas ortogonales,
\begin{align*}
\boxed{
   g_{11} = h_{1}^{2}, \qquad g_{22} = h_{2}^{2}, \qquad g_{33} = h_{3}^{2}
   }
\end{align*}
\end{frame}
\begin{frame}
\frametitle{Una conexión: factores de escala y métrica}
La métrica puede escribirse como:
\pause
\begin{align*}
\boxed{ g_{ij} = \begin{pmatrix}
      h_{1}^{2} & 0 & 0 \\
      0 & h_{2}^{2} & 0 \\
      0 & 0 & h_{3}^{2}
   \end{pmatrix} }
...
Los factores de escala no son solamente factores de conversión:
\textbf{codifican la información geométrica del espacio en el sistema de coordenadas elegido}.
\end{alertblock}
\end{frame}
\begin{frame}
\frametitle{¿Para qué necesitamos los factores de escala?}
\begin{exampleblock}{Aplicación directa: medir distancias}
Una vez conocidos los factores de escala podemos determinar
cómo se mide una distancia infinitesimal en el espacio:
\begin{align*}
   \dd{s^{2}} = g_{ij} \, \dd{u_{i}} \, \dd{u_{j}}
\end{align*}
\end{exampleblock}
\end{frame}
\begin{frame}
\frametitle{Factores de escala y métrica}
Para un sistema ortogonal:
\begin{align*}
\boxed{\dd{s}^{2} = h_{1}^{2} (\dd{u_{1}})^{2} + h_{2}^{2} (\dd{u_{2}})^{2} + h_{3}^{2} (\dd{u_{3}})^{2}}
\end{align*}
\end{frame}
\begin{frame}
\frametitle{Factores de escala y métrica}
\textbf{Ejemplo: coordenadas esféricas} $(r, \theta, \phi)$:
\begin{align*}
...
Por lo tanto,
\begin{align*}
   \boxed{ \dd{s^{2} = (\dd{r})^{2}} + r^{2} (\dd{\theta})^2 + r^{2} \, \sin^{2} \theta \, (\dd{\phi}^{2})
         }
\end{align*}
\end{frame}
\begin{frame}
\frametitle{Factores de escala y métrica}
\begin{alertblock}{¿Qué nos está diciendo el espacio?}
El mismo cambio diferencial $\dd{\theta}$ no representa la misma distancia en todos los puntos del espacio:
\begin{align*}
   \dd{l_{\theta}} = r \, \dd{\theta}
\end{align*}
\pause
La geometría del sistema coordenado está contenida en los factores de escala y, equivalentemente, en la métrica $g_{ij}$.
\end{alertblock}
\end{frame}
\begin{frame}
\frametitle{Vectores unitarios}
