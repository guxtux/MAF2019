...
u_{t} =  \alpha^{2} \,  u_{xx}
\end{align*}
En este ejercicio se trabajará con un problema unidimensional, es decir, la transmisión del calor a lo largo de una barra de longitud $L$.
\end{frame}
\begin{frame}
...
%Ref. Farlow (1993) - Partial Differential Equations for Scientists and Engineers
%Ref. Cap. 5
...
\end{align*}
\pause
donde $X (x)$ es alguna \textocolor{lava}{función que depende solo de} $x$ \pause y $T (t)$ es alguna \textocolor{darkscarlet}{función que depende solo de} $t$.
\end{frame}
\begin{frame}
\frametitle{El por qué de las soluciones sencillas}
Las soluciones son sencillas porque cualquier temperatura $u (x, t)$ de esta forma conservará su \textocolor{darkslateblue}{forma} básica para diferentes valores de tiempo $t$, como podemos ver en la figura (\ref{fig:figura_separacion_variables_01})
\end{frame}
\begin{frame}
...
\end{frame}
\begin{frame}
\frametitle{Obteniendo las derivadas parciales}
La EDP es: $u_{t} = \alpha^{2} \, u_{xx}$
\\
\bigskip
\pause
Entonces:
...
\begin{eqnarray*}
\begin{aligned}
u_{t} = X (x) \, \pderivada{T} (t) \\[0.5em]
u_{x} = \pderivada{X} (x) \, T (t) \\[0.5em]
u_{xx} = \sderivada{X} (x) \, T (t)
\end{aligned}
\end{eqnarray*}
\end{frame}
\begin{frame}
\frametitle{Sustituyendo las derivadas parciales}
Haciendo esta sustitución de las derivadas parciales en la expresión inicial:
\pause
\begin{align*}
...
Para obtener lo que se conoce como \textocolor{blue}{variables separables}, es decir, la expresión del lado izquierdo de la igualdad depende solo de $t$, mientras que la expresión del lado derecho, depende solo de $x$. %\footnote{El primado sencillo indica la diferenciación de primer grado con respecto a la variable señalada, mientras que el primado doble, señala la diferenciación de segundo grado con respecto a la variable que se indica.}
\end{frame}
\begin{frame}
...
\begin{align*}
u (x, t) &= X (x) \, T (t) \\[0.5em]
T (t) &= A \, \exp\left( -\lambda^{2} \alpha^{2} t \right)  \hspace{0.5cm} \text{A constante arbitraria} \\[0.5em]
X (x) &= A \sin (\lambda x ) + B \, \cos (\lambda x) \hspace{0.5cm} \text{(A, B arbitrarias)}
\end{align*}
\end{frame}
\begin{frame}
\frametitle{Resolviendo las EDO}
...
\pause
\begin{align*}
u (x, t) &= \exp\left( -\lambda^{2} \alpha^{2} t \right)  \bigg[  A \sin (\lambda x ) + B \, \cos (\lambda x) \bigg]
\end{align*}
Con $A, B, \lambda$ arbitrarias. \pause En este punto, tenemos un número infinito de funciones que satisfacen la EDP.
\end{frame}
...
\pause
\begin{align*}
X (x) = A \, \sin (a x) + B \, \cos (a x ), & \hspace{0.2cm} \lambda = a^{2}
\end{align*}
\end{frame}
\begin{frame}
\frametitle{Soluciones con las CDF}
...
\pause
\begin{align*}
X (x) = A \, \sin (a x) + B \, \cos (a x) \hspace{2cm}
\end{align*}
\pause
que al sustituir en la solución, multiplicando la solución de $X (t)$ y de $T (t)$:
\begin{eqnarray*}
u (0, t) &=& B \, e^{-\lambda \alpha^{2} \, t} = 0 \pause \hspace{0.3cm} \Longrightarrow \hspace{0.3cm} B = 0 \\[0.5em] \pause
u (L, t) &=& A \, e^{-\lambda \alpha^{2} \, t} \, \sin (\lambda \, L) = 0 \pause \hspace{0.3cm} \Longrightarrow \hspace{0.3cm} \sin (\lambda \, L) = 0
\end{eqnarray*}
\end{frame}
\begin{frame}
...
\begin{align*}
  u (0, y) &= 0 \hspace{1cm} u (1, y) = g (y) \\[0.5em]
  u (x, 0) &= 0  \hspace{1cm} u (x, 1) = 0
\end{align*}
\end{frame}
\begin{frame}
