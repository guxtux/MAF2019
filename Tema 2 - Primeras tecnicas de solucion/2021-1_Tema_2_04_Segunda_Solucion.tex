...
\setbeamercolor{item projected}{bg=arsenic,fg=white}
...
\begin{frame}
\frametitle{Independencia lineal en funciones}
Dicho de otra forma, si la única solución de la ecuación (\ref{eq:ecuacion_09_111}) es $k_{\lambda} = 0$, para toda $\lambda$, el conjunto de funciones $\varphi_{\lambda}$ se dice que es \textocolor{auburn}{linealmente independiente}.
\end{frame}
% \par
...
\end{align}
A este determinante se le llama \textocolor{byzantium}{Wronskiano}:
\end{frame}
\begin{frame}
...
\begin{enumerate}
\item Si el Wronkiano \textocolor{carnelian}{es distinto de cero}, entonces la ecuación (\ref{eq:ecuacion_09_111}) no tiene solución más que $k_{\lambda} = 0$.
\\
...
\frametitle{Resultado del Wronskiano}
 Estas dos soluciones $\varphi_{1}$ y $\varphi_{2}$ son por tanto \textocolor{ceruleanblue}{linealmente independientes}.
 \\
 \bigskip
 \pause
...
\begin{frame}
\frametitle{Caso especial}
Lo que significa que, usando ya sea (\ref{eq:ecuacion_09_127}) o (\ref{eq:ecuacion_09_128}), podemos \textocolor{cordovan}{tomar una solución conocida} y luego integrando, podemos \textocolor{darkgreen}{generar una segunda solución linealmente independiente}.
\end{frame}
\begin{frame}
\frametitle{Ejemplo}
De la EDO2H:
...
\setbeamercolor{item projected}{bg=fielddrab,fg=white}
\setbeamertemplate{enumerate items}{%
\usebeamercolor[bg]{item projected}%
\raisebox{1.5pt}{\colorbox{bg}{\color{fg}\footnotesize\insertenumlabel}}%
}
...
\pause
%Ref. Arfken (2013) Chap. 7 pag. 366
\begin{align}
...
\begin{frame}
\frametitle{La segunda solución}
Como veremos en el Tema de Funciones Especiales, \pause la segunda solución linealmente independiente de la ecuación diferencial de Bessel está dada por la \textocolor{indigo(web)}{función de Neumann} de orden $0$, $N_{0}(x)$:
\end{frame}
\begin{frame}
\frametitle{La segunda solución}
\begin{align}
...
\pause
\setbeamercolor{item projected}{bg=lasallegreen,fg=white}
\setbeamertemplate{enumerate items}{%
\usebeamercolor[bg]{item projected}%
\raisebox{1.5pt}{\colorbox{bg}{\color{fg}\footnotesize\insertenumlabel}}%
}
...
\bigskip
\pause
Una segunda solución linealmente independiente se puede construir por medio de la doble integral Wronskiana. Esto es lo único posible: \textocolor{mediumred-violet}{no hay una tercera solución linealmente independiente}.
\end{frame}
\begin{frame}
\frametitle{Ecuaciones no homogéneas}
La ED2 lineal y \textocolor{red}{no homogénea} tendrá una solución adicional: \pause la \textocolor{navyblue}{solución particular}.
\end{frame}
\begin{frame}
\frametitle{Ecuaciones no homogéneas}
...
\setbeamercolor{item projected}{bg=oldmauve,fg=white}
\setbeamertemplate{enumerate items}{%
\usebeamercolor[bg]{item projected}%
\raisebox{1.5pt}{\colorbox{bg}{\color{fg}\footnotesize\insertenumlabel}}%
}
