...
\end{align*}
Vemos que para el punto $x_{0} = 0$ se tendría algún tipo de singularidad.
\end{frame}
\begin{frame}
\frametitle{Revisión en el caso límite}
Entonces revisamos qué sucede cuando $x_{0} \to 0$:
\pause
\begin{eqnarray*}
\begin{aligned}
    \lim_{x_{0} \to 0} (x - x_{0}) \, P (x) &= \pause \lim_{x_{0} \to 0} \dfrac{ \left( x - x_{0} \right)}{3 \, x} = \pause \dfrac{1}{3} \\[1em] \pause
    \lim_{x_{0} \to 0} (x - x_{0})^{2} \, Q (x) &= \pause \lim_{x_{0} \to 0} - \dfrac{\left( x - x_{0} \right)^{2}}{3 \, x} = \\[1em] \pause
    &= \dfrac{x}{3} 
\end{aligned}
\end{eqnarray*}
\pause
Entonces $x = x_{0}$ satisface la definición de un punto singular regular.
\end{frame}
\begin{frame}
\frametitle{Solución}
...
a_{2} &=& \dfrac{a_{1}}{8 \cdot 2} = \dfrac{a_{0}}{2! \cdot 5 \cdot 8} \\[0.5em] \pause
a_{3} &=& \dfrac{a_{2}}{11 \cdot 3} = \dfrac{a_{0}}{3! \cdot 5 \cdot 8 \cdot 11} \\
\vdots \\[0.5em] \pause
a_{n} &=& \dfrac{a_{0}}{n! \cdot 5 \cdot 8 \cdot 11 \ldots (3\, n + 2)} \hspace{1cm} n = 1, 2, 3, \ldots
\end{eqnarray*}
\end{frame}
\begin{frame}
...
\pause
\begin{align}
y_{1} = a_{0} \, x^{2/3} \left[ 1 + \nsum_{n=1}^{\infty} \dfrac{a_{0}}{n! \cdot 5 \cdot 8 \cdot 11 \ldots (3\, n + 2)} \, x^{n} \right]
...
a_{2} &=& \dfrac{a_{1}}{2 \cdot 4} = \dfrac{a_{0}}{2! \cdot 1 \cdot 4}  \\[0.5em] \pause
a_{3} &=& \dfrac{a_{2}}{3 \cdot 7} = \dfrac{a_{0}}{3! \cdot 4 \cdot 7}  \\[0.5em]
\vdots \\ \pause
a_{n} &=& \dfrac{a_{0}}{n! \cdot 1 \cdot 4 \cdot 7 \ldots (3 \, n - 2)} \hspace{1cm} n = 1, 2, 3, \ldots
\end{eqnarray*}
\end{frame}
\begin{frame}
...
\pause
\begin{align}
y_{2} = a_{0} \, x^{0} \left[ 1 + \nsum_{n=1}^{\infty} \dfrac{1}{n! \cdot 1 \cdot 4 \cdot 7 \ldots (3\, n - 2)} \, x^{n} \right]
...
&= C_{1} \, \left[ x^{2/3} + \nsum_{n=1}^{\infty} \dfrac{a_{0}}{n! \cdot 5 \cdot 8 \cdot 11 \ldots (3\, n + 2)} \, x^{n} \right] + \\[0.5em]
&+ C_{2} \, \left[ 1 + \nsum_{n=1}^{\infty} \dfrac{1}{n! \cdot 1 \cdot 4 \cdot 7 \ldots (3\, n - 2)} \, x^{n} \right]
\end{aligned}
\end{eqnarray*}
\end{frame}
...
\begin{frame}
\frametitle{Caso 2}
\textocolor{lust}{Las raíces difieren en un entero positivo.} \pause Si $r_{1} - r_{2} = N$, donde $N$ es un entero positivo, entonces existen dos soluciones linealmente independientes de la ED, de la forma:
\pause
\begin{subequations}
\begin{align}
...
Vamos a simplifcar nuestros cálculos y nos permitirá demostrar cómo surgen las funciones de Bessel si suponemos que $u$ es una función de $r$, $\theta$ y $t$, \textocolor{red}{pero no una función de $z$}.
\end{frame}
...
Ocupando el método de separación de variables, suponemos que existe una solución para $u$: 
\pause
\begin{align*}
...
\pause
\begin{align*}
\dfrac{1}{K} \, \dfrac{\pderivada{T}}{T} = - \lambda
\end{align*}
...
La ecuación diferencial con respecto a $\Theta$, es:
\begin{align}
...
\end{frame}
\begin{frame}
\frametitle{La ecuación con respecto a $R$}
Para $R$, entonces hacemos:
\pause
\begin{align*}
...
\end{align*}}
En donde vemos que los dos primeros términos los hemos dejado fuera de la suma. 
\end{frame}
\begin{frame}
\frametitle{Ecuación simplificada}
Volvemos a reducir la expresión, para obtener:
\pause
\begin{align*}
...
Entonces para el coeficiente de $x^{r+1}$ se tiene que:
\pause
\begin{eqnarray*}
