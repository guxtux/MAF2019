...
La tercera propiedad importante de un operador autoadjunto (Hermitiano) consiste en que \textocolor{byzantium}{las eigenfunciones forman un conjunto completo}.
\end{frame}
\begin{frame}
\frametitle{La tercera propiedad por revisar}  
...
Con mayor formalismo, el conjunto $\phi_{n} (x)$ se dice que es \textocolor{ao}{completo}, si en el límite el error medio cuadrado se anula:
\pause
\begin{align}
...
\begin{frame}
\frametitle{Funciones válidas}
Un ejemplo relevante es \textocolor{cobalt}{el fenómeno de Gibbs} de las series discontinuas de Fourier, que también ocurre para otras series de funciones propias.
\end{frame}
\begin{frame}
...
\bigskip
\pause
En ocasiones los coeficientes $a_{m}$ son llamados \textocolor{red}{coeficientes generalizados de Fourier}.
\end{frame}
\begin{frame}
\frametitle{Calculando los coeficientes}
Para una función conocida $F (x)$, la ecuación (\ref{eq:ecuacion_10_64}) proporciona los $a_{m}$ como una \textocolor{ao(english)}{integral definida} que siempre se puede evaluar, ya sea numéricamente si es que no es de manera analítica.
\end{frame}
\begin{frame}
...
\bigskip
\pause
Con un producto punto definido por la ec. (\ref{eq:ecuacion_10_64}), el espacio lineal que tenemos, se convierte en un \textocolor{bulgarianrose}{espacio de Hilbert}.
\end{frame}
\begin{frame}
...
Si el conjunto de funciones $\phi_{n} (x)$ \textocolor{burgundy}{no forma un conjunto completo}, \pause posiblemente sea por que no se han incluido el número infinito de elementos del conjunto completo, esto nos conduce a la \textocolor{carmine}{desigualdad de Bessel}.
\end{frame}
\begin{frame}
...
\end{align}
Que es \underline{\textocolor{denim}{la desigualdad de Bessel}}.
\end{frame}
\begin{frame}
...
Cuando se considera que las eigenfunciones que forman conjuntos completos (como los polinomios de Legendre), la ec. (\ref{eq:ecuacion_10_73}) con el signo igual que se mantiene se llamará \textocolor{goldenbrown}{relación de Parseval}.
\\
\bigskip
\pause
...
\begin{frame}
\frametitle{Interpretando la desigualdad}
\setbeamercolor{item projected}{bg=black,fg=ivory}
\setbeamertemplate{enumerate items}{%
\usebeamercolor[bg]{item projected}%
\raisebox{1.5pt}{\colorbox{bg}{\color{fg}\footnotesize\insertenumlabel}}%
}
...
Sustituyendo esos valores de $\lambda$ y $\lambda^{*}$ en la ecuación (\ref{eq:ecuacion_10_79}), \pause obtenemos la ecuación (\ref{eq:ecuacion_10_78}), \underline{\textocolor{lincolngreen}{la desigualdad de Schwarz}}.
\end{frame}
\begin{frame}
