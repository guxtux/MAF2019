...
\begin{align*}
(\psi, \phi) = \scaleint{5ex}_{\bs \bs a}^{b} \psi^{*} (x) \, \phi (x) \dd{x} = 0
\end{align*}
...
% \par
% \textbf{Ejercicio a cuenta: } Escribe las condiciones de ortonormalidad y completes para las siguientes bases de peso unitario:
% \begin{enumerate}[label=\roman*)]
...
Paul Dirac estableció el vector de estado $\psi$ por el símbolo $\ket{\psi}$, al cual llamó vector \emph{ket}, o simplemente ket. Los kets pertenecen al espacio (vectorial) de Hilbert $\mathcal{H}$, o en corto, al espacio de kets.
\par
...
\begin{equation}
\braket{\phi}{\psi}^{*} = \left( \scaleint{5ex} \phi^{*} (\va*{r},t) \, \psi (\va*{r}, t) \dd[3]{x} \right)^{*} = \scaleint{5ex} \psi^{*} (\va*{r}, t) \, \phi (\va*{r}, t) \dd[3]{t} = \braket{\psi}{\phi}
...
\subsection*{Relaciones que involucran funciones de operadores.}
