...
\begin{frame}
\frametitle{Procedimientos más generales}
 Es decir, usamos la separación de variables para separar la EDP dada en un conjunto de EDO.
\end{frame}
\begin{frame}
...
Una clase de problemas a la que pertenecen estos ejemplos anteriores, son \textocolor{red}{los problemas de valores propios de Sturm-Liouville}.
\end{frame}
\begin{frame}
...
\begin{enumerate}[<+->]
\item \textocolor{lava}{Operadores autoadjuntos} (diferenciales) que juegan un papel importante en la teoría espectral de operadores lineales.
\item La \textocolor{coquelicot}{existencia de las eigenfunciones} necesarias para resolver problemas de física descritos por los problemas de VI o CDF anteriores.
\end{enumerate}
\end{frame}
...
En física, muchos problemas surgen en forma de problemas de CDF que involucran EDO2.
\\
\bigskip
\pause
...
\bigskip
\pause
Una mayor separación de variables conduce a un conjunto de problemas de valores de frontera que involucran EDO2.
\end{frame}
\begin{frame}
...
Podemos escribir una ecuación de este tipo en forma de un operador definiendo el \textocolor{coolblack}{operador diferencial}:
\pause
\begin{align*}
...
Veremos que estas ecuaciones se pueden resolver usando \textocolor{byzantium}{expansiones de eigenfunciones}.
\\
\bigskip
\pause
...
\begin{frame}
\frametitle{Propiedad del operador diferencial}
El operador resultante se denomina \textocolor{ao(english)}{operador Sturm-Liouville}.
\\
\bigskip
\pause
...
\begin{frame}
\frametitle{El problema de S-L}    
Si el intervalo es finito y estas suposiciones sobre los coeficientes son verdaderas en $[a, b]$, entonces se dice que el problema es un \textocolor{carmine}{problema regular de Sturm-Liouville}.
\\
\bigskip
\pause
De lo contrario, se denomina \textocolor{cerise}{problema singular de Sturm-Liouville}.
\end{frame}
\begin{frame}
...
El siguiente paso es multiplicar la ecuación diferencial por $\mu (x)$:
\pause
\begin{align*}
...
Entonces la \textocolor{ao}{ortogonalidad} de las eigenfunciones se puede escribir de la forma:
\pause
\begin{align}
...
\begin{enumerate}[<+->]
\conti
\item El conjunto de eigenfunciones es \textocolor{carnelian}{completo}, \pause es decir, cualquier función suave se puede representar mediante una expansión generalizada de las eigenfunciones de la serie de Fourier:
\pause
\begin{align*}
f (x) \sim \nsum_{n=1}^{\infty} c_{n} \, \phi_{n} (x)
\end{align*}
...
Se puede demostrar que tal espacio es isomorfo en un \textocolor{magenta}{espacio de Hilbert}, un espacio de producto interno completo.
\\
\bigskip
\pause
