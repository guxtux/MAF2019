...
\title{\large{Funciones ortogonales}}
...
\begin{frame}
\frametitle{Introducción}
En este tema, nos enfocaremos no a resolver la ED sino a \textocolor{ao}{desarrollar y comprender las propiedades generales de las soluciones}.
\end{frame}
\begin{frame}
\frametitle{Introducción}
...
\begin{frame}
\frametitle{Introducción}
La diagonalización de una matriz simétrica real corresponde en este tema, la solución de una ED definida por un \textocolor{carmine}{operador autoadjunto} $\mathcal{L}$ en términos de sus eigenfunciones, que son el análogo \enquote{continuo} de los eigenvectores.
\end{frame}
\begin{frame}
\frametitle{Introducción}
...
\begin{enumerate}[<+->]
\conti
\item La propiedad general de la \textocolor{bistre}{completes} de un conjunto de eigenfunciones.
\end{enumerate}
\end{frame}
...
\begin{frame}
\frametitle{El operador lineal}
Se requiere que las integrales en las ecs. (\ref{eq:ecuacion_10_02}) y (\ref{eq:ecuacion_10_03}) sean idénticas \textocolor{darkblue}{para todas las funciones $\bm{u}$ (diferenciables dos veces)}, por lo que los integrandos deben de ser iguales.
\end{frame}
\begin{frame}
\frametitle{El operador lineal}
...
\end{aligned}
\end{eqnarray}
Este es el \textocolor{darkmagenta}{operador autoadjunto $\overline{\mathcal{L}}$}.
\end{frame}
\begin{frame}
\frametitle{El operador autoadjunto}
...
\end{equation}
\pause
se dice que $\mathcal{L}$ es un \textocolor{darkscarlet}{operador autoadjunto}. 
\\
\bigskip
\pause
...
Sin embargo, la teoría de las ED autoadjuntas lineales de segundo orden es perfectamente general porque \textocolor{deepcarmine}{siempre podemos transformar} el operador no autoadjunto en la forma autoadjunta requerida.
\end{frame}
\begin{frame}
...
\bigskip
\pause
La eigenfunción $\psi (x)$ puede ser compleja y generalmente se denomina \textocolor{ao}{función de onda}.
\end{frame}
\begin{frame}
...
Para una elección dada del parámetro $\lambda$, una función $u_{\lambda}(x)$, que satisface la ec. (\ref{eq:ecuacion_10_08}) \textocolor{blue}{y las CDF impuestas}, se llama una \textocolor{red}{eigenfunción} (función propia) correspondiente a $\lambda$.
\\
\bigskip
\pause
...
\bigskip
\pause
La función de peso también modifica la definición de \textocolor{lava}{ortogonalidad} de dos funciones propias: \pause son ortogonales si su producto interno $\ip{u_{\pderivada{\lambda}}}{u_{\lambda}} = 0$.
\end{frame}
\begin{frame}
...
\begin{table}[!ht]
%\caption{Tabla con parámetros y coeficientes de ED con eigenvalores.}
\centering
\scriptsize
\begin{threeparttable}
...
El operador $\mathcal{L}$ se llama \textocolor{cobalt}{Hermitiano} si:
\pause
\begin{align}
...
El \textocolor{red}{valor esperado} de un operador $\mathcal{L}$ se define como:
\pause
\begin{align}
...
% \end{equation}
% Suponemos la existencia de un segundo eigenvalor y eigenfunción
% \begin{equation}
...
% \bigskip
% \pause
% Permitimos que tanto $\lambda_{k}$ que son los eigenvalores, así como $u_{k}$ las funciones propias, sean complejos.
% \end{frame}
% \begin{frame}
% \frametitle{Primera propiedad}
...
% \begin{align}
% \begin{aligned}[b]
% \scaleint{6ex}_{\bs a}^{b} u_{j}^{*} \, \mathcal{L} \, u_{i} \dd{x} &- \scaleint{6ex}_{\bs a}^{b} u_{i} \, \mathcal{L}^{*} \, u_{j}^{*} \dd{x} = \\[1em]
% &= (\lambda_{j}^{*} - \lambda_{i}) \, \scaleint{6ex}_{\bs a}^{b}  u_{i} \, u_{j}^{*} \, w \dd{x}
% \end{aligned}
...
% \begin{equation}
% (\lambda_{j}^{*} - \lambda_{i}) \, \scaleint{6ex}_{\bs a}^{b}  u_{i} \, u_{j}^{*} \, w \dd{x} = 0
...
% \begin{frame}
% \frametitle{Primera propiedad}
% Lo que es una prueba matemática de que el eigenvalor es real, ya que $\lambda_{i}$ representa a cualquiera de los eigenvalores, se cumple esta propiedad.
% \end{frame}
% \begin{frame}
% \frametitle{Cantidades medibles}
% En mecánica cuántica los eigenvalores corresponden a cantidades medibles, tales como la energía o el momento angular
% \\
% \bigskip
% Con la teoría revisada en términos de operadores Hermitianos, esta prueba de que los eigenvalores nos garantiza que la teoría predice valores reales para esas cantidades físicas medibles.
% \end{frame}
...
Esta condición es llamada \textocolor{byzantium}{ortogonalidad}, es el análogo continuo del producto escalar de dos vectores y éste se anula.
\\
\bigskip
\pause
...
\pause
\setbeamercolor{item projected}{bg=burntumber,fg=white}
\setbeamertemplate{enumerate items}{%
\usebeamercolor[bg]{item projected}%
\raisebox{1.5pt}{\colorbox{bg}{\color{fg}\footnotesize\insertenumlabel}}%
}
...
\bigskip
\pause
La propiedad de \textocolor{cardinal}{completes} significa que cierta clase de funciones (funciones continuas por secciones o pedazos), puede representarse por una serie de funciones propias ortogonales, con cualquier grado de precisión.
\end{frame}
\begin{frame}
\frametitle{Ejemplo 2}
...
Como regla, las funciones propias forman un \textocolor{red}{Espacio de Hilbert}, que es un espacio vectorial completo de funciones con una métrica definida por el producto punto.
\end{frame}
\begin{frame}
